  ...
  const handleSSOLogin = async (provider: "github" | "google") => {
    ...
    try {
      ...
      if (errorCode === "PROVIDER_NOT_FOUND" || errorCode?.includes("PROVIDER_NOT_FOUND")) {
        ...
        toast.error(t("ssoFailed", { provider: providerLabel }));
      }
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            {isDev && devUsers.length > 0 && (
              <div className="space-y-2">
                <label className="text-sm font-medium">{t("devPickUser")}</label>
                <Select
                  onValueChange={(email) => {
                    setEmail(email);
                    setPassword("password123");
                ...
                >
                  <SelectTrigger className="h-12">
                    <SelectValue placeholder={t("devSelectPlaceholder")} />
                  </SelectTrigger>
                  <SelectContent>
                    {devUsers.map((u) => (
